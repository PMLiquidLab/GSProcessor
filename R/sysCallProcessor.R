...
#' @export
#'
sysCallProcessor <- function( input_folder.dir = NA , output_folder.dir = NA ,  tmp_folder.dir=NA,
                       RscriptPathName = NA,
  ...
  start <- function() {
    ...
    while( get.current.state() =="RUNNING" ) {
      ...
      tmpZipExtractionFolder <- paste(c("zip.",format(Sys.Date(), format = "%Y%m%d%H%m%s"),"_",runif(n = 1,0,100000)),collapse = '')
      ...
      runFolder <- paste(c(global.lst.tmp_folder$path,"/",paste(c("tIUID_",current.tokenInstanceUID),collapse = '')),collapse = '')
      dir.create(runFolder)
      if(!dir.exists(runFolder)) { stop("ERORR: token run folder not created")}
            
      # Ora che hai costruito le cartelle per il run, copiaci dentro i file presenti nella cartella dello zip
      # (il contenuto della cartella 'zipTmpFolder' deve finire in 'runFolder')
      # RIMUOVI la cartella nello zip temporaneo
      ...
      for( run.id in names(res$lst.run)) {
        ...
        lst.parameter <- list("header"=list("run.id"=run.id,"currentDir"=runFolder,"executionFolderFullName"=executionFolderFullName,"tokenInstanceUID"=tokenInstanceUID),"payload"=res)
        ...
        # ZIP del risultato
        # -----------------------------------------------------------------------
        zipFile <- paste( c( runFolder,"/",tokenInstanceUID,".zip"), collapse = '')
      ...
      # PULIZIA
      # -----------------------------------------------------------------------
      unlink(runFolder, recursive = TRUE)
  ...
  openZippedPackage <- function( fileName , fullFileName , zipTmpFolder) {
    ...
    # Crea la cartella tmp dove aprire lo zip
